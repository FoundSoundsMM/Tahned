...
work = sys.argv[1]
fails = 0
print("\n  effect    peak     t=2s     t=6s     t=12s    verdict")
print("  " + "-" * 58)
for name in ("reverb", "delay", "chorus"):
    p = os.path.join(work, f"fx-{name}.wav")
    if not os.path.exists(p):
        print(f"  {name:9s} NOT RENDERED"); fails += 1; continue
